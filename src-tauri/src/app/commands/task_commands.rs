...
pub async fn create_task(
    ...
    debug!(
        "[Command] create_task, title: '{}', project_id: {:?}, parent_id: {:?}",
...
pub async fn get_tasks_by_parent(
    ...
    debug!(
        "[Command] get_tasks_by_parent, project_id: {:?}, parent_id: {:?}",
...
pub async fn get_task_by_id(id: i64, state: tauri::State<'_, AppState>) -> Result<Task> {
    debug!("[Command] get_task_by_id, id: {}", id);
    let conn = state.db.lock().unwrap();
...
pub async fn update_task_priority(
    ...
    debug!(
        "[Command] update_task_priority, id: {}, priority: {:?}",
        id, priority
    );
    // 成功时无需返回数据
...
pub async fn update_task_description(
    ...
    debug!("[Command] update_task_description, id: {}", id);
    let conn = state.db.lock().unwrap();
    task_queries::update_task_description(&conn, id, description)?;
    info!("[Command] 成功更新任务描述, ID: {}", id);
    Ok(())
}
